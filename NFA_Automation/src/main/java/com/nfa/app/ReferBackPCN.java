...
public class ReferBackPCN {
	...
	public void ReferBackPCNRequest(String username, String password) throws InterruptedException {
		...
		if(ApproverHomePage.noRecordFound != null || ApproverHomePage.noRecordFound == null && ApproverHomePage.noRecordFound != null) {		
			...
			Thread.sleep(2000);
			((JavascriptExecutor)WebDriver_Instance.driver).executeScript("arguments[0].scrollIntoView();", ApproverHomePage.reviewPCNReferBack);
			
			Thread.sleep(3000);
			Action.click(ApproverHomePage.reviewPCNReferBack);
			
			if(ApproverHomePage.reviewPCNReferRadioButton != null)
			{
				Action.click(ApproverHomePage.reviewPCNReferRadioButton);
				
				Thread.sleep(2000);
				Action.click(ApproverHomePage.reviewPCNRefereRadioButtonContinue);
				
				Thread.sleep(3000);
				String alertmgs = ApproverHomePage.reviewPCNRefereRadioButtonContinueAlert.getText();
				System.out.println("Alert : " + alertmgs);
				
					if(alertmgs.contains("An error occured while updating details. Kindly try after some time!")) {
						
						Action.click(ApproverHomePage.reviewPCNRefereRadioButtonContinueAlertOk);
						
						Thread.sleep(2000);					
						Action.click(ApproverHomePage.reviewPCNReferRadioButtonClose);
